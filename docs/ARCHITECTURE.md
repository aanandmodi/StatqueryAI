...
flowchart LR
    ...
    F -->|free edge route| E[Bounded Sites model proxy]
    E --> Q[Qwen3-VL ZeroGPU Space]
    F -. local / scale-up .-> B[Python FastAPI controller]
...
| Public controller | Sites edge route | Same-origin validation/proxy; no separate paid service |
| Full controller | Local/tested FastAPI service | Preserves raster validation, audit PDF and scale-up contract |
...
The public demo uses one ZeroGPU Space plus a Sites edge API, leaving the second eventual free slot
for a released change/fusion specialist. The Python controller remains the stronger local/future
deployment profile and keeps the same specialist output contract.
